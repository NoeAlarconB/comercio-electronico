...
<body>
		...
		<div class="flex w-full justify-center p-4 gap-4 flex-wrap">
			<% if (productos != null && !productos.isEmpty()) { %>
				<% for(Producto producto: productos) { %>
					<div class="flex bg-gray-200 hover:scale-103 transition transform duration-300 w-[220px] 
						...
						<a href="/comercio-electronico/detalleProducto?idProducto=<%= producto.getIdProducto() %>">
							<img class="rounded-sm" src="<%= producto.getImagenUrl() %>" width="250px">
						</a>
						<span class="text-center font-bold"><%= producto.getNombre() %></span>
						<span class="w-full text-left"> S/ <%= producto.getPrecio() %></span>
						<button class="bg-[rgb(0,180,100)] font-bold text-white px-6 py-1 rounded-full text-center cursor-pointer hover:bg-green-400" 
	...
	<script>
		function agregarCarrito(idProducto) {
			...
			fetch('/comercio-electronico/carrito?idProducto=' + idProducto, {
				method:'POST'
			});
		}
	</script>
