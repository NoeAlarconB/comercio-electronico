...
<body>
    ...
    <%
		List<PedidoDetalle> detalles = (List<PedidoDetalle>) request.getAttribute("detallesCarrito");
	%>
	...
	<div class="p-4">
		<h1 class="text-center text-cyan-500 text-2xl font-bold mb-4">MI CARRITO POSEIDON STORE</h1>
		<% if (detalles != null && !detalles.isEmpty()) { %>
			<table class="w-full table-auto border-collapse border border-gray-300">
				<thead class="bg-gray-300">
					<tr>
						<th class="border px-3 py-3">ID Producto</th>
						<th class="border px-3 py-3">Producto</th>
						<th class="border px-3 py-3">Imagen</th>
						<th class="border px-3 py-3">Cantidad</th>
						<th class="border px-3 py-3">Precio</th>
					</tr>
				</thead>
				<tbody>
					<% for (PedidoDetalle detalle : detalles) { %>
						<tr>
							<td class="border px-4 py-3 "><%= detalle.getIdProducto() %></td>
							<td class="border px-4 py-3"><%= detalle.getNombreProd() %></td>
							<td class="text-center"><img src = "<%= detalle.getImagenUrl() %>" class="w-12 h-12 object-cover inline-block text-align"></td>
							<td class="border px-4 py-3 text-center"><%= detalle.getCantidad() %></td>
							<td class="border px-4 py-3 text-center">S/. <%= detalle.getPrecio() %></td>
						</tr>
					<% } %>
				</tbody>
			</table>
